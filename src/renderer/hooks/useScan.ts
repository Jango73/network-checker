...
export const useScan = () => {
  ...
  > => {
    ...
    const suspiciousFolders = ruleset.datasets.suspiciousFolders?.values || [];
  ...
  const scanNetwork = useCallback(async (useFakeConnection: boolean) => {
    setIsScanning(true);
    setScanResults([]);
    ...
    try {
      ...
      for (const conn of rawConnections) {
        ...
        if ((conn as any).__meta) {
          ...
          processPath = normalizePathToUnix(pathResp.success ? pathResp.data : '');
        ...
        const context = {
          ...
          nonRiskyHistoryCount: history.filter(
            (h: HistoryEntry) =>
              h.process.toLowerCase() === processName.toLowerCase() &&
              !h.isRisky
          ).length,
          config,
        ...
        if (isRisky && !hasAlerted) {
          ...
          audio
            .play()
            .catch(err => console.error('Failed to play alert:', err));
          hasAlerted = true;
        }
