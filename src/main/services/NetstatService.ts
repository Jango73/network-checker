...
export class NetstatService {
    ...
    async getConnections(): Promise<Connection[]> {
        try {
            ...
            throw new Error(`Failed to execute netstat: ${(error as Error).message}`);
        }
    }
    ...
    private parseNetstatOutput(output: string): Connection[] {
        ...
        for (const line of lines) {
            ...
            if (match) {
                const [, protocol, localAddrPort, remoteAddrPort, state = '', pid] = match;

                // Split address and port
                const localMatch = localAddrPort.match(/(\[?[\w:.*%]+\]?):(\d+|\*)/);
                const remoteMatch = remoteAddrPort.match(/(\[?[\w:.*%]+\]?):(\d+|\*)/);
                ...
                if (localMatch && remoteMatch) {
                    ...
                    localAddress = localAddress.replace(/[\[\]]/g, '').replace(/%\d+$/, '');
                    remoteAddress = remoteAddress.replace(/[\[\]]/g, '').replace(/%\d+$/, '');
                    ...
                    if (
                        ...
                        if (!uniqueConnections.has(key)) {
                            uniqueConnections.set(key, {
                                ...
                                remotePort: remotePort === '*' ? 0 : parseInt(remotePort, 10),
                                state: state.trim(),
                                pid: parseInt(pid, 10),
                            });
                        }
